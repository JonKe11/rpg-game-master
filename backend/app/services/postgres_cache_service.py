...
- Deduplication in bulk upsert
- Complete scraping log method
- ✅ NEW: JSONB query functions for location hierarchy
...
class PostgresCacheService:
    ...
    def get_distinct_jsonb_values(
        ...
        Used to build the location hierarchy (e.g., get all 'Sectors' 
        where 'Region' is 'Outer Rim Territories').
        
        Args:
            universe: Universe name
            category: Category to search in (e.g., "planets")
            field: The JSONB key to get distinct values from (e.g., "Sector")
            filters: Dict of filters to apply (e.g., {"Region": "Outer Rim"})
    ...
    def get_articles_by_jsonb_filters(
        ...
        Used to get planets in a specific system, or locations on a planet.
        
        Args:
            universe: Universe name
            category: Category to search in (e.g., "planets", "locations")
            filters: Dict of filters to apply (e.g., {"System": "Tatoo system"})
            with_images: Only return articles that have an image_url
            limit: Max results
