...
# ⛔️ USUNIĘTO: from app.core.scraper.wiki_scraper import WikiScraper
# ✅ NOWE IMPORTY:
...
class AdaptiveGameMaster:
    ...
    - Używa Wiki (z bazy PostgreSQL) jako fundamentu kanonu
    ...
    def __init__(self, model_name: str = "llama3.1:8b"):
        ...
        try:
            ...
            logger.info("✅ AdaptiveGameMaster połączony z PostgresCacheService")
        except Exception as e:
            logger.error(f"❌ AdaptiveGameMaster nie mógł połączyć się z DB: {e}")
            self.db = None
            self.pg_cache = None
        
        # Cache dla danych z Wiki (w pamięci podręcznej AGM)
        self.wiki_cache = {}
...
ZASADY ABSOLUTNE:
1. KANON z Wiki: Używaj TYLKO dostarczonych danych z Wiki. planety, rasy, technologie, historia - NIGDY nie zmieniaj.
2. MOŻESZ tworzyć: 
...
- Pamiętaj o konsekwencjach poprzednich decyzji"""
    ...
    def __del__(self):
        # Upewnij się, że zamykamy sesję bazy danych
        if self.db:
            self.db.close()
    ...
    def start_session(self, character: Dict, universe: str = None) -> Dict:
        ...
        self.story_states[session_id] = self._init_story_state()
        
        # Pobierz dane o rasie postaci z Wiki
        race_data = None
        if character.get('race'):
            # ✅ Używamy nowej metody z kategorią
            race_data = self._get_wiki_data(character['race'], universe, 'species')
    ...
    def _generate_unique_intro(self, character: Dict, race_data: Dict, universe: str) -> Dict:
        ...
        # ✅ Używamy nowej metody z kategorią
        location_data = self._get_wiki_data(location, universe, 'planets')
...
NIE rozpoczynaj akcji, tylko ustaw scenę."""
    ...
    def _parse_action(self, action: str) -> Tuple[ActionType, List[str]]:
        ...
        if re.search(r'\b[ABC]\)|wybier|opcj', action): action_type = ActionType.CHOICE
        elif re.search(r'\b(idź|iść|przejdź|wejdź|udaj|rusz|bieg|leć)\b', action_lower): action_type = ActionType.MOVE
        elif re.search(r'\b(patrz|zobacz|rozglą|obserw|sprawdź|zbadaj)\b', action_lower): action_type = ActionType.EXAMINE
        elif re.search(r'\b(mów|powiedz|pytaj|rozmaw|gadaj|zapytaj)\b', action_lower): action_type = ActionType.TALK
        elif re.search(r'\b(walcz|atakuj|bij|strzel|broń)\b', action_lower): action_type = ActionType.COMBAT
        elif re.search(r'\b(weź|podnieś|użyj|kup|sprzedaj|otwórz)\b', action_lower): action_type = ActionType.INTERACT
        else: action_type = ActionType.INTERACT
        
        entities = re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', action)
    ...
    def _determine_narrative_style(self, action_type: ActionType, story_state: Dict, context: Dict) -> NarrativeStyle:
        ...
        if action_type == ActionType.CHOICE: return NarrativeStyle.CHOICES
        ...
        if action_type == ActionType.COMBAT or tension > 8: return NarrativeStyle.CINEMATIC
        if action_type == ActionType.TALK: return NarrativeStyle.GUIDED
...
Akcja gracza: {action}
Stwórz interesującą sytuację z 3 różnymi opcjami (A, B, C).
Format odpowiedzi:
...
C) [Opcja kreatywna/nieoczywista]"""
        ...
        return {
            'message': response, 'type': 'choice',
            'narrative_style': 'choices', 'timestamp': datetime.now().isoformat()
        }
...
Akcja gracza: {action}
Stwórz DYNAMICZNĄ, kinematyczną scenę (3-5 zdań). Zakończ cliffhangerem."""

        response = self._generate_llm_response(prompt, universe)
        effects = self._add_scene_effects(action)
        
        return {
            'message': response, 'type': 'cinematic',
            'narrative_style': 'cinematic', 'effects': effects,
            'timestamp': datetime.now().isoformat()
        }
...
Akcja gracza: {action}
Opisz rezultat dając graczowi PEŁNĄ swobodę. Stwórz nowego NPC lub odkryj nowe miejsce (w ramach kanonu)."""

        response = self._generate_llm_response(prompt, universe)
        return {
            'message': response, 'type': 'exploration',
            'narrative_style': 'open_world', 'timestamp': datetime.now().isoformat()
        }
...
Opisz rezultat i DELIKATNIE zasugeruj możliwe kierunki akcji."""
        ...
        response = self._generate_llm_response(prompt, universe)
        return {
            'message': response, 'type': 'guided',
            'narrative_style': 'guided', 'hints': hints,
            'timestamp': datetime.now().isoformat()
        }
    ...
    def _create_npc(self, universe: str, role: str = None) -> Dict:
        ...
        personalities = ["nerwowy i podejrzliwy", "pewny siebie i arogancki", "przyjazny ale przebiegły"]
        motivations = ["szuka łatwego zarobku", "ukrywa mroczną tajemnicę", "ma dług do spłacenia"]
        occupations = {'star_wars': ['przemytnik', 'pilot', 'mechanik'], 'lotr': ['karczmarz', 'kupiec'], 'default': ['mieszkaniec']}
    ...
    def _generate_npc_name(self, universe: str) -> str:
        ...
        if universe == 'star_wars':
            prefixes = ["Zar", "Kor", "Jax", "Ven", "Dar"]
            suffixes = ["ek", "an", "is", "on", "ax"]
            return f"{random.choice(prefixes)}{random.choice(suffixes)}"
        elif universe == 'lotr':
            prefixes = ["Brom", "Gar", "Thal", "Dun"]
            suffixes = ["dir", "mond", "wick", "stone"]
            return f"{random.choice(prefixes)}{random.choice(suffixes)}"
        else:
            return f"NPC_{random.randint(100, 999)}"
    
    def _get_canon_races(self, universe: str) -> List[str]:
        """✅ ZMODYFIKOWANE: Pobiera listę kanonicznych ras z PostgreSQL"""
        cache_key = f"{universe}_races"
        if cache_key in self.wiki_cache:
            return self.wiki_cache[cache_key]
        
        if not self.pg_cache:
            logger.warning("Brak pg_cache, używam domyślnej listy ras.")
            return ['Human', 'Twi\'lek', 'Rodian']

        try:
            # Użyj nowej funkcji z postgres_cache_service
            results = self.pg_cache.search_articles_paginated(
                universe=universe,
                category="species",
                limit=200 # 200 najpopularniejszych ras
            )
            races = [article.title for article in results['items']]
            
            if not races:
                raise Exception("Baza danych nie zwróciła ras.")
                
            self.wiki_cache[cache_key] = races
            return races
            
        except Exception as e:
            logger.error(f"Nie udało się pobrać ras z PostgreSQL: {e}, używam domyślnej listy.")
            fallback = ['Human', 'Twi\'lek', 'Rodian', 'Zabrak', 'Duros', 'Mon Calamari']
            self.wiki_cache[cache_key] = fallback
            return fallback

    def _get_canon_starting_locations(self, universe: str) -> List[str]:
        """✅ ZMODYFIKOWANE: Pobiera listę kanonicznych lokacji startowych z PostgreSQL"""
        cache_key = f"{universe}_planets"
        if cache_key in self.wiki_cache:
            return self.wiki_cache[cache_key]
        
        if not self.pg_cache:
            logger.warning("Brak pg_cache, używam domyślnej listy lokacji.")
            return ['Tatooine', 'Coruscant']
        
        try:
            # Pobierz 50 najpopularniejszych planet
            results = self.pg_cache.search_articles_paginated(
                universe=universe,
                category="planets",
                limit=50
            )
            locations = [article.title for article in results['items']]
            
            if not locations:
                raise Exception("Baza danych nie zwróciła planet.")
                
            self.wiki_cache[cache_key] = locations
            return locations
            
        except Exception as e:
            logger.error(f"Nie udało się pobrać planet z PostgreSQL: {e}, używam domyślnej listy.")
            fallback = ['Tatooine', 'Coruscant', 'Naboo', 'Corellia', 'Nar Shaddaa', 'Lothal']
            self.wiki_cache[cache_key] = fallback
            return fallback
    ...
    def _update_story_state(self, action: str, action_type: ActionType, story_state: Dict):
        ...
        if story_state['tension_level'] > 7: story_state['current_beat'] = StoryBeat.CLIMAX
        elif story_state['tension_level'] > 4: story_state['current_beat'] = StoryBeat.CONFLICT
        elif story_state['tension_level'] > 2: story_state['current_beat'] = StoryBeat.BUILD_UP
    ...
    def _fetch_wiki_data_for_entities(self, entities: List[str], universe: str) -> Dict:
        ...
        wiki_data = {}
        for entity in entities[:5]: # Max 5 encji
            # ✅ ZMODYFIKOWANE: Nie wiemy jaka jest kategoria, więc szukamy ogólnie
            data = self._get_wiki_data(entity, universe, 'characters') # Spróbuj 'characters'
            if not data:
                data = self._get_wiki_data(entity, universe, 'planets') # Spróbuj 'planets'
            if not data:
                data = self._get_wiki_data(entity, universe, 'locations') # Spróbuj 'locations'
                
            if data:
                wiki_data[entity] = data
        return wiki_data
    
    def _get_wiki_data(self, entity: str, universe: str, category: str) -> Optional[Dict]:
        """✅ ZMODYFIKOWANE: Pobiera dane z cache lub PostgreSQL"""
        cache_key = f"{universe}:{category}:{entity.lower()}"
        if cache_key in self.wiki_cache:
            return self.wiki_cache[cache_key]
        ...
        try:
            # Użyj nowej, bezpośredniej metody
            article = self.pg_cache.get_article_by_title(
                title=entity,
                universe=universe
            )
            if article:
                # Sprawdź, czy kategoria pasuje (opcjonalne, ale dobre dla trafności)
                # if article.category != category:
                #     return None # Znaleziono, ale to nie ta kategoria
                ...
                data = {
                    ...
                    'info_box': article.content
                }
                self.wiki_cache[cache_key] = data
                return data
        except Exception as e:
            logger.error(f"Błąd PostgreSQL przy pobieraniu _get_wiki_data dla {entity}: {e}")
    ...
    def generate_dice_roll(self, dice_type: str = 'd20') -> Dict:
        ...
        dice_values = {'d4': 4, 'd6': 6, 'd8': 8, 'd10': 10, 'd12': 12, 'd20': 20, 'd100': 100}
        ...
        return {
            ...
            'result': roll,
            'critical': critical,
            'message': f"Rzut {dice_type}: {roll}" + (f" - Krytyczny {'sukces' if critical == 'success' else 'porażka'}!" if critical else "")
        }
    ...
    def _generate_llm_response(self, prompt: str, universe: str) -> str:
        ...
        try:
            response = self.client.generate(
                ...
                prompt=full_prompt,
                options={'temperature': 0.7, 'top_p': 0.9, 'max_tokens': 400}
            )
    ...
    def _check_ollama_connection(self) -> bool:
        ...
        try:
            ...
            logger.warning("⚠️ Nie można połączyć się z Ollama.")
            return False