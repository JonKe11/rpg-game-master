...
function CampaignList({ character, onCreateNew, onJoinCampaign }) {
  ...
  // ✅ NOWA FUNKCJA: Renderowanie przycisku akcji
  const renderJoinButton = (campaign) => {
    ...
    if (campaign.status === 'lobby') {
      return (
        ...
        >
          {isParticipant ? "Return to Lobby" : "Join Lobby"}
        </button>
      );
    }
    ...
    // Dla 'completed' itp.
    return (
      <button disabled className="w-full bg-gray-600 px-4 py-2 rounded-lg cursor-not-allowed">
        {campaign.status}
      </button>
    );
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={onCreateNew} className="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg font-semibold transition">
          + Create New Campaign
        </button>
      </div>
      ...
      {campaigns.length === 0 ? (
        <div className="text-center py-12 bg-gray-800 rounded-lg">
          ...
          <button onClick={onCreateNew} className="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-lg font-semibold transition">
        ...
        <div className="max-h-[calc(100vh-250px)] overflow-y-auto pr-2">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {campaigns.map((campaign) => (
              <div key={campaign.id} className="bg-gray-800 rounded-lg p-6 hover:bg-gray-750 transition relative">
                
                {campaign.creator_id === currentUserId && campaign.status === 'lobby' && (
                  <button
                    onClick={(e) => { e.stopPropagation(); handleDeleteCampaign(campaign.id); }}
                    className="absolute top-2 right-14 bg-red-600 hover:bg-red-700 p-2 rounded-lg transition text-sm"
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-xl font-bold text-blue-400">{campaign.title}</h4>
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      campaign.status === 'lobby' ? 'bg-yellow-600' :
                      campaign.status === 'active' ? 'bg-green-600' :
                      campaign.status === 'paused' ? 'bg-gray-600' : 'bg-gray-600'
                    }`}>
                      {campaign.status}
                    </span>
                  </div>
                  <div className="space-y-1 text-sm">
                    <p className="text-gray-400">Universe: <span className="text-white capitalize">{campaign.universe.replace('_', ' ')}</span></p>
                    <p className="text-gray-400">Players: <span className="text-white">{campaign.player_count}/{campaign.max_players}</span></p>
                    <p className="text-gray-400">GM: {campaign.has_gm ? <span className="text-green-400">✓ Assigned</span> : <span className="text-red-400">✗ Needed</span>}</p>
                  </div>
                </div>

                {/* ✅ Użyj nowej funkcji do renderowania przycisku */}
